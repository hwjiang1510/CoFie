...
def create_mesh(decoder, latent_vec, latent_vec_valid, latent_vec_valid_idx,
                cube_size, box_size, filename, N=128, max_batch=32 ** 3, offset=None, scale=None):
    start = time.time()
    ...
    samples[:,:3] = generate_grid_center_indices(N, 1.0).reshape(-1,3)
    ...
    sdf_grid_indices = deep_ls.data_voxel.generate_grid_center_indices(cube_size, box_size).reshape(-1,3).numpy()
    ...
    for center_point_index in tqdm(range(len(sdf_grid_indices_valid))):
        near_sample_indices = sdf_tree_first.query_radius([sdf_grid_indices_valid[center_point_index]], grid_radius)
        num_sdf_samples = len(near_sample_indices[0])
        if num_sdf_samples < 1: 
            continue
        near_sample_indices = near_sample_indices[0]
        near_sample_indices_two = sdf_tree_second.query_radius([sdf_grid_indices_valid[center_point_index]], grid_radius)
